...
function DoubtCard({ doubt, onResolve }: { doubt: Doubt; onResolve: (id: string) => void }) {
 ...
 return (
  <div className={`bg-white border rounded-md overflow-hidden ${
   ...
   <button
    onClick={() => setOpen((o) => !o)}
    className="w-full flex items-start gap-4 px-5 py-4 text-left hover:bg-gray-50/50 "
   >
  ...
  <div className="max-w-3xl">
   ...
   <div className="flex items-center justify-between mb-6">
    ...
    <button
     ...
     className="flex items-center gap-2 bg-blue-600 text-white font-semibold px-3 py-1.5 rounded-md hover:bg-blue-700 text-sm"
    >
   ...
   <div className="flex gap-1 bg-gray-100 rounded-md p-1 mb-6 w-fit">
    {(["all", "pending", "answered", "resolved"] as const).map((f) => (
     <button
      key={f}
      onClick={() => setFilter(f)}
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium capitalize ${
